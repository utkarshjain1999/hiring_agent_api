...
def match_resumes_service(request: MatchingRequest):
    stored_df = fetch_latest_resumes()
    if stored_df.empty:
    ...
    top_matches = get_top_matching_resumes(parsed_df, request.jd_text, request.threshold, request.top_n)
...
def get_top_matching_resumes(df, jd_text, threshold, top_n):
